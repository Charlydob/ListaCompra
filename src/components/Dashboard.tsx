...
import type { GuardDataRepository } from '../data/storage/repository'
import { defaultProfile, type Profile } from '../types'
import { Settings } from './Settings'

const money = new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' })
export function Dashboard({ repository, mode, onExit }: { repository: GuardDataRepository; mode: 'local' | 'firebase'; onExit: () => void }) {
  ...
  useEffect(() => { repository.loadProfile().then(setProfile).catch(() => setSaveError('No se pudo cargar el perfil.')).finally(() => setLoading(false)) }, [repository])
  useEffect(() => { setLoading(true); repository.loadGuards(monthKey).then(setGuards).catch(() => setSaveError('No se pudieron cargar las guardias.')).finally(() => setLoading(false)) }, [repository, monthKey])
  ...
  const toggle = async (date: string) => { const next = guards.includes(date) ? guards.filter((item) => item !== date) : [...guards, date].sort(); setGuards(next); setSaveError(''); try { await repository.saveGuards(monthKey, next) } catch { setGuards(guards); setSaveError('No se pudo guardar el cambio.') } }
  const persistProfile = async (next: Profile) => { await repository.saveProfile(next); setProfile(next) }
  ...
  return <main className="app-shell" onTouchStart={(e) => { touchX.current = e.touches[0].clientX }} onTouchEnd={(e) => { const delta = e.changedTouches[0].clientX - touchX.current; if (Math.abs(delta) > 70) setMonth(delta < 0 ? addMonths(month, 1) : subMonths(month, 1)) }}>
    <nav>{mode === 'local' && <span className="local-badge">Modo local</span>}<button className="settings-button" onClick={() => setSettings(true)}>⚙ Ajustes</button><button className="logout" onClick={onExit}>Salir</button></nav>
